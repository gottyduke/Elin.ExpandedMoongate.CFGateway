...
export async function handleMapsTop(
  ...
  if (sort === "rating") {
    const minVotes = 20;
    const global = await env.DB.prepare(`SELECT AVG(score) as c FROM map_ratings`)
      .first<{ c: number | null }>();
    const C = global?.c ?? 3.5;

    const stmt = env.DB.prepare(`
      SELECT
        id AS Id, author AS Author, title AS Title, lang AS Lang, cat AS Cat,
        created_at AS CreatedAt, version AS Version, tag AS Tag, is_official AS IsOfficial,
        visit_count AS VisitCount, rating_count AS RatingCount, rating_average AS RatingAverage,
